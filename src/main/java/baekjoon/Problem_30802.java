...
public class Problem_30802 implements ProblemInterface {
    ...
    public void solution(String[] args) throws IOException {
        ...
        StringTokenizer numberOfApplicantsBySize = new StringTokenizer(br.readLine());
        StringTokenizer st = new StringTokenizer(br.readLine());
        int T = Integer.parseInt(st.nextToken());
        int P = Integer.parseInt(st.nextToken());
        final int SIZE_COUNT = 6;

        int numberOfTshirtBundle = 0;
        for (int i = 0; i < SIZE_COUNT; i++) {
            int people = Integer.parseInt(numberOfApplicantsBySize.nextToken());
            int bundles = people / T;
            if (people % T != 0) {
                bundles++;
            }
            numberOfTshirtBundle += bundles;
        }

        System.out.println(numberOfTshirtBundle);
